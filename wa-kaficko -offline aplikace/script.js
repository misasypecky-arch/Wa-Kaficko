...
async function saveData() {
    ...
    try {
        ...
        openModal("✅ Uloženo!", "<p>Vaše káva byla úspěšně zapsána do systému.</p>");
        resetCounts();
    } catch (err) {
        saveToOfflineQueue(payload);
        resetCounts();
...
function saveToOfflineQueue(payload) {
    ...
    openModal("💾 Uloženo lokálně", "<p>Jste offline nebo je API nedostupné.</p><p><b>Záznam čeká v mobilu</b> a odešle se automaticky, jakmile se připojíte.</p>");
}
...
function showDailySummary() {
    ...
    if (!summary || summary.date !== today || Object.keys(summary.drinks).length === 0) {
        openModal("Dnešní přehled", "<p>Dnes jste si zatím žádnou kávu nezaznamenali. Běžte to napravit! ☕</p>");
        return;
    }
    ...
    for (const [type, count] of Object.entries(summary.drinks)) {
        html += `<li>${type} <span>${count}x</span></li>`;
        total += count;
    }
    html += `</ul>`;
    html += `<p style="margin-top: 15px; font-weight: bold; border-top: 2px solid #eaeaea; padding-top: 10px;">Celkem: ${total} nápojů</p>`;

    openModal("☕ Dnešní skóre", html);
}
